#ifndef HV_DTLS_CLIENT_HPP_
#define HV_DTLS_CLIENT_HPP_
...
class DtlsClientEventLoopTmpl {
public:
    typedef std::shared_ptr<TSocketChannel> TSocketChannelPtr;

    DtlsClientEventLoopTmpl(EventLoopPtr loop = NULL) {
        loop_ = loop ? loop : std::make_shared<EventLoop>();
        remote_port = 0;
    }

    virtual ~DtlsClientEventLoopTmpl() {
    }
    ...
    int createsocket(int remote_port, const char* remote_host = "127.0.0.1") {
        hio_t* io = hloop_create_dtls_client(loop_->loop(), remote_host, remote_port);
        if (io == NULL) return -1;
    ...
    // start thread-safe
    void start() {
        loop_->runInLoop(std::bind(&DtlsClientEventLoopTmpl::startRecv, this));
    }
...
template<class TSocketChannel = SocketChannel>
class DtlsClientTmpl : private EventLoopThread, public DtlsClientEventLoopTmpl<TSocketChannel> {
public:
    DtlsClientTmpl(EventLoopPtr loop = NULL)
        : EventLoopThread(loop)
        , DtlsClientEventLoopTmpl<TSocketChannel>(EventLoopThread::loop())
        , is_loop_owner(loop == NULL)
    {}
    virtual ~DtlsClientTmpl() {
        stop(true);
    }
    ...
    void start(bool wait_threads_started = true) {
        if (isRunning()) {
            DtlsClientEventLoopTmpl<TSocketChannel>::start();
        } else {
            EventLoopThread::start(wait_threads_started, std::bind(&DtlsClientTmpl::startRecv, this));
        }
    }

    // stop thread-safe
    void stop(bool wait_threads_stopped = true) {
        DtlsClientEventLoopTmpl<TSocketChannel>::closesocket();
        if (is_loop_owner) {
...
typedef DtlsClientTmpl<SocketChannel> DtlsClient;

}

#endif // HV_DTLS_CLIENT_HPP_
